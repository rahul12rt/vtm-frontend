    ...
    const fetchResults = async () => {
      try {
        ...
        const response = await fetch(
          `${strapiApiUrl}/api/results?filters[create_test][exam_type][$eq]=3&filters[student][college][user_name][$eq]=${collegeUsername}&populate[create_test][populate][subject][populate]=class&filters[create_test][exam_name][$eq]=${stream.toUpperCase()}`
        );
        ...
        {subjects.length > 0 ? (
          subjects.map((subject, index) => (
            <div
              ...
              onClick={() => router.push(`/college-portal/date-section-exam/${stream}/${subject.toLowerCase()}`)}
            >
              <h3>{subject}</h3>
            </div>
          ))
          ...
          className={styles.card}
          onClick={() =>
            router.push(`/college-portal/date-section-exam/${stream}/all`)
          }
        >
          <h3>All</h3>
        </div>
