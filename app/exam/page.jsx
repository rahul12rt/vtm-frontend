...
const ExamList = () => {
  ...
  useEffect(() => {
    ...
    const fetchAssignedTests = async (classId, academicYear, collegeId, streams) => {
      try {
        const strapiApiUrl = process.env.NEXT_PUBLIC_STRAPI_API_URL;
        const testsEndpoint = `${strapiApiUrl}/api/assign-tests?filters[Assign][$eq]=true&filters[create_test][class][id][$eq]=${classId}&filters[create_test][academic_year][year][$eq]=${academicYear}&filters[create_test][exam_type][$eq]=3&filters[create_test][exam_name][$eq]=${streams}&filters[colleges][id][$eq]=${collegeId}&populate=create_test.class,create_test.academic_year,create_test.duration,create_test.subject,create_test.exam_type`;
  ...
  return (
    <div className="container">
      ...
      {filteredTests.length > 0 ? (
        ...
        <div>No {filter === "all" ? "" : ""} exam found.</div>
      )}
    </div>
  );
};
