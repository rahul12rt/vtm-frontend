...
function ExamResults() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        const resultsEndpoint = `${strapiApiUrl}/api/results?filters[create_test][exam_type][$eq]=3&filters[student][user_name][$eq]=${username}&populate[create_test][populate]=class,academic_year,subject,topics,exam_name,exam_type,college&populate=student`;
  ...
  return (
    <div className="container">
      <div className="sectionHeader">Exam Results</div>
          ...
          return (
            <div key={result.id} className="examResult">
              ...
              <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
                <h4 style={{ margin: 0 }}>Name : {createTest.exam_name}</h4> |
                <h4 style={{ margin: 0 }}>Date: {createTest.date}</h4>
              </div>
