  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const selfStudyEndpoint = `${strapiApiUrl}/api/assing-self-study-to-colleges?populate[self_studies][populate][subject][populate][class]=*&populate[self_studies][populate][material]=*&populate[college]=*&filters[college][id][$eq]=${collegeId}&filters[self_studies][subject][class][id][$eq]=${classId}`;
        const selfStudyResponse = await fetch(selfStudyEndpoint);
        if (!selfStudyResponse.ok)
          throw new Error("Failed to fetch self-study materials");
    ...
    <div className="container">
      ...
      {selfStudyMaterials.length === 0 ? (
        <div className={styles.noMaterials}>
          No self-study materials assigned. Please check back later or contact
          your administrator.
        </div>
      ) : (
        <div className={styles.grid}>
          {selfStudyMaterials.map((item) =>
            item.attributes.self_studies.data.map((study) => (
