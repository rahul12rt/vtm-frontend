...
export async function GET() {
  try {
    ...
    const strapiEndpoint = `${strapiApiUrl}/api/qualifications`;
...
export async function DELETE(request) {
  try {
    ...
    const strapiEndpoint = `${strapiApiUrl}/api/qualifications/${qualificationId}`;
    const response = await fetch(strapiEndpoint, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
...
export async function PUT(request) {
  try {
    ...
    const { qualificationId, name } = await request.json();

    if (!qualificationId || !name) {
      return NextResponse.json(
        { error: "Qualification ID and name are required" },
        { status: 400 }
      );
    }
